...
response = GET(path="/admin", _host=COLLABORATOR_URL, _lab=_lab, session=session)
print(response.status)
for name, header in response.headers.items():
    print(f"{name}: {header}")
response_body = response.read().decode()
print(response_body)
if response.status != 200 or 'collaborator' not in response.headers['server']:
